...
async function fetchData(filter) {
  ...
  try {
    const response = await fetch(
      `https://localhost:7165/api/Plantacoes/${filter}`
    );
    if (!response.ok) {
      displayErrorModal("Erro ao buscar plantações");
    }
...
async function search() {
  ...
  let url = `https://localhost:7165/api/Plantacoes`;

  if (id) {
    url += `/${id}`;
  } else if (name) {
    url += `/nome?nome=${encodeURIComponent(name)}`;
    ...
    displayErrorModal(
      "Por favor, insira um ID, Nome ou intervalo de datas para pesquisa."
    );
    hideLoading();
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      displayErrorModal("Plantações não encontradas");
    }
    const data = await response.json();
    if (Array.isArray(data) && data.length === 0) {
      document.getElementById(
        "reportTableContainer"
      ).innerHTML = `<p class="no-data-message">Nenhuma plantação encontrada com o ${
        id
          ? `ID ${id}`
          : name
          ? `Nome "${name}"`
...
function populateTable(data, title) {
  ...
  container.innerHTML = `<h3>Relatório de Plantação - ${title}</h3>`;
...
function generatePdf() {
  ...
  // Salvar o PDF
  doc.save("relatorio_plantacoes.pdf");
}
