...
async function fetchData(filter) {
  ...
  try {
    const response = await fetch(
      `https://localhost:7165/api/Insumos/${filter}`
    );
    if (!response.ok) {
...
async function search() {
  ...
  try {
    ...
    if (!response.ok) {
      displayErrorModal("Insumos não encontrados");
    }
    const data = await response.json();
    if (Array.isArray(data) && data.length === 0) {
      document.getElementById(
        "reportTableContainer"
      ).innerHTML = `<p class="no-data-message">Nenhum insumo encontrado com o ${
        id ? `ID ${id}` : `Nome "${name}"`
      }.</p>`;
    } else if (!Array.isArray(data)) {
      // Se a resposta for um objeto (pesquisa por ID), transformá-la em um array
      populateTable([data], id ? `ID: ${id}` : `Nome: ${name}`);
    } else {
      populateTable(data, id ? `ID: ${id}` : `Nome: ${name}`);
    }
  } catch (error) {
...
function populateTable(data, title) {
  ...
  data.forEach((item) => {
    ...
    const cells = [
      ...
      item.nomeDoInsumo,
      item.funcao,
      item.fornecedor.nomeDoFornecedor + " - " + item.fornecedor.cnpj,
      formatDate(item.dataValidade),
      formatDate(item.dataEntrada),
      item.quantidadeEntrada + "ml",
      item.mililitrosAtual + "ml",
      item.status,
    ];
    cells.forEach((text) => {
  ...
  container.innerHTML = `<h3>Relatório de Insumo - ${title}</h3>`;
