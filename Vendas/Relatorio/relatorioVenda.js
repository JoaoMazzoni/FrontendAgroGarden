...
async function fetchData(filter) {
  showLoading();
  try {
    const response = await fetch(`https://localhost:7165/api/Vendas/${filter}`);
    if (!response.ok) {
      displayErrorModal("Erro ao buscar insumos");
    }
...
async function search() {
  ...
  const cnpj = document.getElementById("searchCnpj").value.trim();
  ...
  if (id) {
    ...
    url += `cliente/${encodeURIComponent(cnpj)}`;
  } else if (cpf) {
    url += `funcionario/${encodeURIComponent(cpf)}`;
  }
  try {
    const response = await fetch(url);
    if (!response.ok) {
      displayErrorModal("Vendas não encontradas");
    }
    const data = await response.json();
    if (Array.isArray(data) && data.length === 0) {
      document.getElementById(
        "reportTableContainer"
      ).innerHTML = `<p class="no-data-message">Nenhuma venda encontrada com o ${`ID ${id}`}.</p>`;
    } else if (data.id) {
...
async function LoadVendas() {
  ...
  try {
    ...
    if (Array.isArray(data) && data.length === 0) {
      document.getElementById(
        "reportTableContainer"
      ).innerHTML = `<p class="no-data-message">Nenhum venda encontrada.</p>`;
    } else if (data.id) {
      // Se a resposta for um objeto (pesquisa por ID), transformá-la em um array
      populateTable([data], `Todas as vendas`);
    } else {
      populateTable(data, `Todas as vendas`);
    }
  } catch (error) {
    document.getElementById(
      "reportTableContainer"
    ).innerHTML = `<p class="no-data-message">Nenhuma venda encontrada.</p>`;
  } finally {
    hideLoading();
  }
}
...
function populateTable(data, title) {
  ...
  data.forEach((item) => {
    ...
    const cells = [
      item.id,
      item.cliente.razaoSocial + " - " + item.cliente.cnpj,
      item.funcionario.nomeCompleto + " - " + item.funcionario.cpf,
      item.produtos.map(
        (p) => " " + p.produto.nomeProduto + " - " + p.quantidade
      ),
      item.dataDaVenda,
      item.formaPagamento,
      "R$ " + item.valorTotal,
  ...
  container.innerHTML = `<h3>Relatório de Vendas - ${title}</h3>`;
