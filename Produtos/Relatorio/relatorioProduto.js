...
async function fetchData(filter) {
  ...
  try {
    const response = await fetch(
      `https://localhost:7165/api/Produtos/${filter}`
    );
    if (!response.ok) {
...
async function search() {
  ...
  if (lote) {
    url += `${encodeURIComponent(lote)}`;
  }
  try {
    const response = await fetch(url);
    if (!response.ok) {
      displayErrorModal("Produtos não encontrados");
    }
    const data = await response.json();
    if (Array.isArray(data) && data.length === 0) {
      document.getElementById(
        "reportTableContainer"
      ).innerHTML = `<p class="no-data-message">Nenhum produto encontrado com o ${
        lote ? `Lote "${lote}"` : `entre ${startDate} e ${endDate}`
      }.</p>`;
    } else if (data.lote) {
      // Se a resposta for um objeto (pesquisa por ID), transformá-la em um array
      populateTable(
        [data],
        lote ? `Lote: ${lote}` : `Data entre ${startDate} e ${endDate}`
      );
    } else {
      populateTable(
        data,
...
function populateTable(data, title) {
  ...
  data.forEach((item) => {
    ...
    const cells = [
      ...
      item.colheitaOrigem.numeroRegistro,
      item.quantidade,
  ...
  container.innerHTML = `<h3>Relatório de Produtos - ${title}</h3>`;
...
function generatePdf() {
  ...
  // Salvar o PDF
  doc.save("relatorio_produto.pdf");
